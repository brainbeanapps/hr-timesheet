...
class AccountAnalyticLine(models.Model):
    ...
    @api.multi
    def action_refresh_timetracker(self):
        return {
            'type': 'ir.actions.act_view_reload',
        }
    ...
    def _timesheet_postprocess_values(self, values):
        ...
        if any([x in values for x in self._unit_amount_depencencies()]):
            for line in self:
                if line.project_id and line.is_timetracked:
                        ...
                        if float_compare(
                            ...
                            result[line.id].update({
                                'timetracker_stopped_at': (
                                    self._timetracker_stopped_at_for(
                                        unit_amount
                                    )
                                ),
                            })
    ...
    def _timetracker_started_at_rounded(self):
        if not self.timetracker_started_at:
            return 0
    ...
    def _timetracker_stopped_at_rounded(self):
        if not self.timetracker_stopped_at:
            return 0
